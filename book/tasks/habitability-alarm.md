...
*Verified against `src/domain/catalog.ts`:523 on every lint — no quote, no object.*
...
*Generated by `tools/vault/emit.mjs` from `src/domain/catalog.ts`:523. **Never edit this page** — it is a view, not an artifact. Fix the source and re-compile (`npm run book`).*
